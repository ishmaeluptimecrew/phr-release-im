...
# Response headers sent on every request. SOLVED — Challenge 06 fills this in.
#
# Every one of these clears a specific ZAP Baseline alert. The CSP is the fussy
# one: default-src does not cover form-action or base-uri, so rule 10055 stays
# lit until you name them explicitly.
SECURITY_HEADERS = {
...
class PHRHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/":
            ...
            # SOLVED. This used to return traceback.format_exc() straight to
            # the caller — the deck's "GET /api/error?x=1 returns full Python
            # stack trace". The trace goes to the log, where the on-call
            # engineer can read it; the caller gets a reference and nothing else.
            try:
                raise RuntimeError("record lookup failed for cohort=respiratory-2026")
            except RuntimeError:
                print(traceback.format_exc())
                self._json(500, {"error": "request could not be completed", "ref": "phr-0000"})
        else:
